...
SAMPLE_PROMPTS = [
    "MLS-1234 매물에 대한 정보를 알려주세요.",
    "Fannie Mae의 조립식 주택(Manufactured Housing) 대출 자격 요건은 무엇인가요?",
    "MLS-1234 매물을 10% 계약금으로 30년 대출을 고려하고 있습니다. 월 소득 9,300달러, 지출 3,700달러로 이 매물에 자격이 되나요?",

    { "income": 80000, "totalDebt": 1000, "loanTerm": 30, "loanAmount": 600000, "creditScore": 750, "mlsId": "MLS-3456" },
    { "income": 80000, "totalDebt": 5000, "loanTerm": 30, "loanAmount": 10000, "creditScore": 750, "mlsId": "MLS-3456" }
]
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # 푸터
        st.divider()
        st.write("Amazon Bedrock 채팅봇 애플리케이션")
...
def render_response_mode_selector():
    ...
    st.session_state.response_mode = st.radio(
        "응답 모드 선택",
        ["Foundation Model", "Converse API", "Knowledge Base (Retrieve)", 
         "Knowledge Base (Retrieve & Generate)", "Agent", "Flow"],
        index=0,
...
def render_sample_prompts():
    ...
    for i, prompt in enumerate(SAMPLE_PROMPTS):
        ...
        # 딕셔너리인 경우 JSON 문자열로 표시
        if isinstance(prompt, dict):
            # 버튼에 표시할 간략한 라벨
            button_label = f"JSON 입력 예제 {i+1}"
            ...
            # 추가로 JSON 예시 내용 보여주기 (접을 수 있음)
            with st.expander(f"JSON 예제 {i+1} 내용 보기"):
                st.json(prompt)
        else:
            # 문자열인 경우 기존대로 처리
