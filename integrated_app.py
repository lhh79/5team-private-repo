...
def show_home():
    ...
    with col_status1:
        # 현재 모드 표시
        current_mode = st.session_state.get("response_mode", "Foundation Model")
        st.info(f"현재 모드: **{current_mode}**", icon="ℹ️")
    
    with col_status2:
        # 처리 중 상태 표시
        if st.session_state.get("processing_status", {}).get("is_processing"):
    ...
    # 사용자 입력 필드 및 처리 로직
    user_input = st.chat_input("메시지를 입력하세요...", disabled=st.session_state.get("processing_status", {}).get("is_processing", False))
    if user_input:
